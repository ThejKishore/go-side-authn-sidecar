...
import (
	...
	"encoding/json"
	"log"
	authorization2 "reverseProxy/internal/ingress/authorization"
	"reverseProxy/internal/ingress/jwtauth"
	"reverseProxy/internal/ingress/util"
	"strings"
...
func Handler(c fiber.Ctx) error {
	...
	if err := json.Unmarshal(c.Body(), &bodyData); err != nil {
		// Not all requests have a body (e.g., GET requests)
		bodyData = make(map[string]interface{})
	}

	reqInfo := authorization2.RequestInfo{
		Method:  c.Method(),
		Path:    c.Path(),
	...
	go func() {
		allow, reason, err := authorization2.CheckCoarseAccess(reqInfo, principal)
		coarseCh <- authResult{allow: allow, reason: reason, err: err}
	}()

	go func() {
		// Pass bodyData to CheckFineGrainAccess
		// If bodyData is provided, it will route to plainId for FGA
		allow, reason, err := authorization2.CheckFineGrainAccess(reqInfo, principal, bodyData)
...
// extractHeaders extracts all headers from the Fiber context
